...
public class Main {
    ...
    static void makeChangeGreedy (HashMap<Coin,Integer> result, int amount, List<Coin> coins) throws NoChange {
            ...
            try {
                Coin coin = coins.getFirst();
                result.put(coin, amount / coin.getValue());
                makeChangeSearch(result, amount % coin.getValue(), coins.getRest());
            }
            catch (NoSuchElementE e) {
                if (amount !=0) throw new NoChange();
        }
    }
    ...
    static void makeChangeSearch (HashMap<Coin,Integer> result, int amount, List<Coin> coins) throws NoChange {
        ...
        try {
            ...
            if (coin.getValue() <= amount) {
                try {
                    int count =  result.containsKey(coin) ? result.get(coin) : 0;
                    result.put(coin, ++count);
                    makeChangeSearch(result, amount - coin.getValue(), coins);
                }
                catch(Exception NoSuchElementE) {
                     try {
