...
class LineWrap {
  ...
  */
  ...
  Pair<String,Integer> dpTD(List<String> words, int space) throws NoSuchElementE  {
      Pair<String, Integer> sub1;
      Pair<String, Integer> newLine = new Pair<>("",0);
      Pair<String, Integer> same = new Pair<>("", 0);
      int cost1=0,cost2=0;
      try{
          String w = words.getFirst();
          if (words.getFirst().length() +1 >= space) {
              same = dpTD(words.getRest(), space - w.length() -1);
              newLine = dpTD(words.getRest(), lineWidth - w.length());
              cost1 += same.getSnd();
              cost2 += newLine.getSnd() + Math.pow(space,3);
            if (cost1 >= cost2) {
                return new Pair<>("\n" + words.getFirst() + newLine.getFst(), cost2);
            }
            else {
                return new Pair<>(" "+ words.getFirst() + same.getFst(), cost1);
            }
          }
          else {
            newLine = dpTD(words.getRest(),lineWidth - words.getFirst().length());
            return new Pair<>("\n" + words.getFirst() + newLine.getFst(), cost2);
          }
  ...
  static String dpBU (String s, int lineWidth) {
      ...
      int numW = word.length;
      for (int i=0; i<lineWidth; i++) {
//          hash.put(new Pair<>(lineWidth, i));

      }
      for (int index = lineWidth-1; lineWidth >=0; index --) {
          String w = word[index];
      }
